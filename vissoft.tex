...
\section{Visualizing Service Performance}
  ...
  After redeploying the API, the dashboard can now automatically detect the current version of the project, and can group measurements by this criterion. Based on this data, \tool can generate the view in Figure \ref{fig:tee} where the performance of the give endpoint is tallied by version.
  ...
  This way the maintainer could confirm that the performance of the translation endpoint improved: in the latest version (bottom-most box plot in Figure \ref{fig:tee}) we see the entire boxplot move to the left and there being less outliers.
