...
\mltp{Can we do with only these lines and w/o a config? Then we'll introduce the extra config later, as it becomes needed}
...
\section{Overall Endpoint Utilization}
  ...
  One can see that the service at its peak has about 2500 hits per day and gain insight into the way users interact with the platform since the endpoints are indicators of different activity types, e.g.: 
  ...
  This visualization also provides feedback to the maintainer into which endpoints can be deprecated and which can not. In our case study, the maintainer decided to not remove several endpoints once they saw that contrary to their expectations, the endpoints were being used \footnote{Usage information can also be used to increase the confidence of the maintainer that a given endpoint is not used}
  ...
  Figure \ref{fig:dp} shows one clear pattern: the users of the Zeeguu API do not practice foreign languages as night. Besides that, the API is used around the clock with usually a few hundred hits per hour. So the load currently is not that high. 
